...
option.setUseExternalCheckout(true);
```
...
实现 `TapsbookSDKCallback` 方法来调用您自己的订单提交页面。
```java
@Override
...
```	
 - `CheckoutView`, 是你的app的订单提交界面 这个订单界面需要整合第三方支付平台工具，比如微信或支付宝，导引用户最终完成订单的支付. 当用户完成支付后，即可完成app下单用户界面层的工作。
